...
void read_wav_header(std::ifstream& fs, WavHeader& header, bool verbose) {
  ...
  fs.read(buffer4, 4);
  header.chunk_size = little_to_big_endian_4((unsigned char *)buffer4);
  if(verbose)
  ...
  fs.read(buffer4, sizeof(buffer4));
  header.subchunk1_size = little_to_big_endian_4((unsigned char *)buffer4);
  if(verbose)
    std::cout << "[16-19] Subchunk1 Size:" << header.subchunk1_size << std::endl;

  fs.read(buffer2, sizeof(buffer2));
  header.audio_format = little_to_big_endian_2((unsigned char *)buffer2);
  if(verbose) {
  ...
  fs.read(buffer2, sizeof(buffer2));
  header.num_channels = little_to_big_endian_2((unsigned char *)buffer2);
  if(verbose)
    std::cout << "[22-23] Num Channels: "<< header.num_channels << std::endl;

  fs.read(buffer4, sizeof(buffer4));
  header.sample_rate = little_to_big_endian_4((unsigned char *)buffer4);
  if(verbose)
    std::cout << "[24-27] Sample Rate: " << header.sample_rate << std::endl;

  fs.read(buffer4, sizeof(buffer4));
  header.byte_rate = little_to_big_endian_4((unsigned char *)buffer4);
  if(verbose)
    std::cout << "[28-31] Byte Rate: " << header.byte_rate << " Bit Rate: " << header.byte_rate*8 << std::endl;

  fs.read(buffer2, sizeof(buffer2));
  header.block_align = little_to_big_endian_2((unsigned char *)buffer2);
  if(verbose)
    std::cout << "[32-33] Block Align: " << header.block_align << std::endl;

  fs.read(buffer2, sizeof(buffer2));
  header.bits_per_sample = little_to_big_endian_2((unsigned char *)buffer2);
  if(verbose)
  ...
  fs.read(buffer4, sizeof(buffer4));
  header.subchunk2_size = little_to_big_endian_4((unsigned char *)buffer4);
  if(verbose)
...
void read_pcm_wav_data(std::ifstream& fs, WavHeader& header, long num_samples, long size_of_each_sample, std::vector<Cpx<double>>& out) {
  ...
  for (size_t i=0; i<num_samples; i++) {
    ...
    switch(bytes_in_each_channel) {
      case 4:
        data_in_channel = (data_buffer[offset] & 0x00ff) | ((data_buffer[offset + 1] & 0x00ff) << 8)
                          | ((data_buffer[offset + 2] & 0x00ff) << 16) | (data_buffer[offset + 3] << 24);
        break;
      case 2:
